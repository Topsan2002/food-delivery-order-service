pipeline {
    ...
    environment {
          ...
          ECR_REPO_NAME = 'food-delivery-ordering-service'  // Replace with your ECR repository name
          IMAGE_TAG = "latest"
          WORKSPACE = "/var/lib/jenkins/workspace/food-delivery-ordering-service"
    }
    stages {
        stage('Clone Repo to Build') {
            steps {
                git url: 'https://github.com/Topsan2002/food-delivery-order-service.git',
                    branch:'master'
            }
        }
        ...
        stage('Deploy or Run Docker Container') {
            ...
            steps {
                script {
                    def composeFile = "/var/jenkins_home/workspace/food-delivery/docker-compose/food-delivery-docker-compose/docker-compose.yml"
